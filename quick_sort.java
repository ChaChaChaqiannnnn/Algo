...
public class quick_sort {
    ...
    public static void main(String[] args) {
        if (args.length < 3) {
            System.out.println("Usage: java quick_sort <csv_file> <start_row> <end_row>");
            return;
        }
        ...
        String logFile = String.format("quick_sort_%d_%d.txt", startRow, endRow);
        ...
        System.out.println("Quick sort steps written to " + logFile);

        // Save sorted data (integer + string) to CSV
        String outputCsvFile = "quick_sort_1000.csv";
        try (BufferedWriter csvWriter = new BufferedWriter(new FileWriter(outputCsvFile))) {
            for (Data d : arr) {
                csvWriter.write(d.key + "," + d.val);
                csvWriter.newLine();
            }
        } catch (IOException e) {
            System.err.println("Error writing sorted CSV file: " + e.getMessage());
            return;
        }
        System.out.println("Sorted data saved to " + outputCsvFile);

        // Print sorted array keys on console
